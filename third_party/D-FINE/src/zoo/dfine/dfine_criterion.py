...
class DFINECriterion(nn.Module):
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, values=None):
        ...
        if self.class_weight is not None:
            cw = self.class_weight.to(src_logits.device, dtype=src_logits.dtype)
            if cw.numel() != src_logits.shape[-1]:
                raise ValueError(
                    f"class_weight length ({cw.numel()}) must equal num_classes ({src_logits.shape[-1]})."
                )
            weight = weight * cw.view(1, 1, -1)
